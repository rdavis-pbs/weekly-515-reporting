...
The root folder is set as `OUTPUT_ROOT` in `${CLAUDE_PLUGIN_ROOT}/shared/config.md` (git-ignored
local config; copy `config.example.md` to `config.md` if it doesn't exist). The full path is:
